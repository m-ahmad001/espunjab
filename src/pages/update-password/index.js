...
const ForgotPassword = () => {
  ...
  const handleReset = async e => {
    e.preventDefault()
    console.log('🚀 ~ handleReset ~ e:', e.target[0].value)
    const new_password = e.target[0].value

    const { data, error } = await supabase.auth.updateUser({ password: new_password })
    console.log('🚀 ~ handleReset ~ data, error:', data, error)
    if (error) {
      return toast.error(error.message)
    }
    toast('Passowrd Reset. Please login')
  }
      ...
      <RightWrapper sx={skin === 'bordered' && !hidden ? { borderLeft: `1px solid ${theme.palette.divider}` } : {}}>
        ...
        >
          <BoxWrapper>
            ...
            <Box sx={{ mb: 6 }}>
              <TypographyStyled variant='h5'>Update Password? 🔒</TypographyStyled>
              <Typography variant='body2'>Enter your new password .</Typography>
            </Box>
            <form autoComplete='off' onSubmit={handleReset}>
              <TextField autoFocus required label='Password' sx={{ display: 'flex', mb: 4 }} />
